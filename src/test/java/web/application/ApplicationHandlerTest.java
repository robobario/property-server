...
public class ApplicationHandlerTest {
    ...
    public void getApplication() throws Exception {
        ...
        this.mockMvc.perform(get(Routes.to().application().applicationDetails(APPLICATION_NAME)).accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
                .andExpect(content().contentType(new MediaType(MediaType.APPLICATION_JSON, ImmutableMap.of("charset", "UTF-8")))).andExpect(content()
                .string(new ObjectMapper().writeValueAsString(applicationView)));
    }
    ...
    public void putProperty() throws Exception {
        ...
        ApplicationView applicationView = ViewCreator.createApplicationView(application);
        this.mockMvc.perform(put(Routes.to().application().addProperty(APPLICATION_NAME, "p1", "p2")).accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
                .andExpect(content().contentType(new MediaType(MediaType.APPLICATION_JSON, ImmutableMap.of("charset", "UTF-8")))).andExpect(content()
                .string(new ObjectMapper().writeValueAsString(applicationView)));
    }
    ...
    public void removeProperty() throws Exception {
        ...
        ApplicationView applicationView = ViewCreator.createApplicationView(application);
        this.mockMvc.perform(delete(Routes.to().application().removeProperty(APPLICATION_NAME, "p1")).accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
                .andExpect(content().contentType(new MediaType(MediaType.APPLICATION_JSON, ImmutableMap.of("charset", "UTF-8")))).andExpect(content()
                .string(new ObjectMapper().writeValueAsString(applicationView)));
    }
